...
public abstract class MyBaseAdapter<T> extends BaseAdapter {
	...
	public Object getItem(int position) {
		return null == datas ? null : datas.size() > position ? datas
				.get(position) : null;
	}
	...
	public View getView(int position, View convertView, ViewGroup parent,
			int layoutId) {
		FiterViewHolder viewHolder = FiterViewHolder.get(position, convertView,
				parent, layoutId, mContext);
		convert(viewHolder, datas.get(position));
		return viewHolder.getmContentView();
	}

	protected abstract void convert(FiterViewHolder viewHolder, T t);
}
